...
This directory contains JSON fixtures captured from real GlickoStrategy evaluations. These fixtures provide deterministic test data for unit tests.

## Fixture Files

- `glicko-scoring-gen1.json` - Evaluation data from generation 1
- `glicko-scoring-gen3.json` - Evaluation data from generation 3 (shows progression)

## Regenerating Fixtures

The fixtures are generated by skipped tests in `generator.test.ts`. To regenerate:
...
- **results**: Array of evaluation results for each genome
  - Components (seedScore, environmentScore, glickoScore, conservativeScore, innovationScore)
